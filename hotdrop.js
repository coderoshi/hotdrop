...
function putDropListValueInField(data_field, new_str, len) {
...
// {
// field_id:'search',
// drop_id:'search_suggest', // if doesn't exist, create after field_id
// }
...
// match_not_found:function(){},
// match_finder:function(){},
// match_highlighter:function(){},
// }
...
function HotDataSource(params) {
  ...
  this.match_finder = function( prefix ) {
    ...
    for(var i=0; i<this.hot_data.length; i++) {
      ...
      if(this.hot_data[i].match("^"+prefix)==prefix) {
        matches.push(cc);
      }
    }
    return matches;
    ...
    if( !data || data.length <= 0 ) {
      ...
      jQuery.each(data, function( i, item ) {
        ...
        d.click( function() {
          ...
          putDropListValueInField(data_field, this.childNodes[0].nodeValue, self.hot_drop.input.length);
  ...
  this.attach = function() {
    ...
    jQuery(this.field_selector).keydown(function(e) {
      ...
      if( key_code == KEY_CR || key_code == KEY_TAB ) {
        ...
        jQuery( self.field_selector ).val(v.substring(0, v.length-input.length) + _hot_current_value + ", ");
        self.clear_drop( self.drop_selector );
        return false;
      }
